<?php
	...
	$results = array();
	foreach($_POST['image'] as $i => $name){
	    $result = "An unknown error occured.";
	      try{
		if(substr($_POST['image'][$i],0,5) == "data:"){
			if($_POST['image']['size'][$i] <= $maxsize){
				$result = '1';
				...
				$img_name = $DB->esc($_FILES['image']['name'][$i]);
				if($img_info[0]<=1 || $img_info[1]<=1){
					$result = "Invalid image.";
				}
				else{
					$DB->query("INSERT INTO `images` (`GalleryID`,`Date`,`Content`,`Type`,`Filename`) VALUES('$gallery_id','".time()."','$img_data','$img_type','$img_name')");
					if($DB->error() != ""){
