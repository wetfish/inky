<?php
	...
	function title(){
		...
		$authkey = "";
		if(isset($_GET['a'])) $authkey = $_GET['a'];
	...
	function head_inject(){
	 global $DB,$session_id,$session_id_esc;
         ...
         list($gallery_id,$gallery_name,$gallery_date,$gallery_exp,$gallery_owner,$gallery_privacy,$gallery_authkey,$gallery_mode,$gallery_init) = $DB->fetch($q);
	 if($gallery_name == "") $gallery_name = "Untitled gallery";
	...
	function body(){
		...
		list($gallery_id,$gallery_name,$gallery_date,$gallery_exp,$gallery_owner,$gallery_privacy,$gallery_authkey,$gallery_mode,$gallery_init,$gallery_hits) = $DB->fetch($q);
		if($gallery_privacy && $gallery_authkey != "" && $gallery_owner != $session_id && $gallery_authkey != $authkey){
			header("Location: /?p=401");
			exit();
		}
		...
		if($gallery_owner != $session_id){
			$DB->query("UPDATE `galleries` SET `Hitcount` = `Hitcount` + 1 WHERE `Index` = '$gallery_id'");
		}
?>
	<?php
	...
	<div class="container block">
		<h3 class="cont_head">
			...
			if(!$gallery_init){
			?>
			<script type="text/javascript">$(document).ready(function(){slidefade('editForm',$('.settingsLink').get(0));});</script>
			<?php
				$DB->query("UPDATE `galleries` SET `Initialized`='1' WHERE `Index`='$gallery_id'");
			}
			?>
		</h3>
		<div class="cont_body">
			<?php
				...
				if($gallery_owner == $session_id){
			?>
			<!--<p><?php echo $gallery_hits; ?> hits</p>-->
			<p class='gmeta'>
				<?php echo "<span class='date'>Created " . date("r",$gallery_date) . ".</span>"; ?>
				<?php echo formatHits($gallery_hits,$gallery_date); ?>
			</p>
	...
	<div class="container block">
		...
		<div class="cont_body">
			<?php
				...
				$q = $DB->query("SELECT `Index`,`Date`,`Filename` FROM `images` WHERE `GalleryID`='$gallery_id' ORDER BY `Index` ASC");
				if($DB->count($q)){
			?>
			<div class="image_cont">
			<?php
					while(list($image_i,$image_date,$image_name) = $DB->fetch($q)){
						if($image_name == "") $image_name = "image.jpg";
						if($gallery_mode == "thumb"){
			?>
				<div class="image">
					<div>
					<a href="http://static.inky.ws/image/<?php echo $image_i.$urlAdditional;?>/<?php echo $image_name;?>" id="image_<?php echo $image_i;?>"><img src="http://static.inky.ws/thumb/<?php echo $image_i.$urlAdditional;?>/thumb.jpg" alt="uploaded image"/></a>
					</div>
					...
					<?php } ?>
					<a href="http://static.inky.ws/image/<?php echo $image_i.$urlAdditional;?>/<?php echo $image_name;?>" target="_blank">Hotlink</a>
					</span>
			...
			?>
					...
					<div>
						<b class='imgtitle'><?php echo $image_name;?></b>
					</div>
                                        <div>
     					<a href="http://static.inky.ws/image/<?php echo $image_i.$urlAdditional;?>/<?php echo $image_name;?>" id="image_<?php echo $image_i;?>" target="_blank"><img src="http://static.inky.ws/image/<?php echo $image_i.$urlAdditional;?>/image.jpg" alt="uploaded image"/></a>
                                        </div>
                                        <span><?php echo date("M d, H:i T",$image_date);?></span>
                                        <span class='delete'>
                                        <?php if($gallery_owner == $session_id){ ?>
